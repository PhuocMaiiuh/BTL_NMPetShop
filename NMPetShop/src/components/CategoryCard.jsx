...
const CategoryCard = ({ icon, title, description, link, color }) => {
  ...
  return (
    <Link
      ...
      onMouseEnter={e => {
        e.currentTarget.style.boxShadow = `0 20px 60px rgba(0,0,0,0.1), 0 0 40px ${glowColor.replace('0.25', '0.1')}`;
    ...
    >
      ...
      <h3 className="text-lg font-black text-[#1a3c5e] mb-2">{title}</h3>
      <p className="text-gray-500 text-xs leading-relaxed max-w-[200px] mb-6">{description}</p>
