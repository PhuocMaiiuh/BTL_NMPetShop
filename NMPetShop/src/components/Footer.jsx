...
const Footer = () => {
  ...
  return (
    ...
    >
      {/* Top glow accents (Soft for light theme) */}
      <div
        className="absolute top-0 left-1/4 w-96 h-48 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse, rgba(26,60,94,0.05) 0%, transparent 70%)', filter: 'blur(40px)' }}
      />
      
      {/* Floating paws (Soft gray) */}
      {[
        { top: '15%', left: '3%', size: 18, op: 0.03 },
        { top: '60%', left: '8%', size: 12, op: 0.02 },
        { top: '25%', right: '4%', size: 20, op: 0.03 },
        { top: '75%', right: '6%', size: 14, op: 0.02 },
      ].map((p, i) => (
        <FaPaw
          key={i}
          size={p.size}
          className="absolute pointer-events-none animate-float"
          style={{ top: p.top, left: p.left, right: p.right, opacity: p.op, color: '#1a3c5e', animationDelay: `${i * 0.8}s` }}
        />
      ))}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 mb-14">
          ...
          <div className="lg:col-span-4">
            <Link to="/" className="inline-flex items-center gap-3 mb-5 group">
              ...
              <div>
                <p className="text-lg font-black text-[#1a3c5e] leading-none">NM Pet Shop</p>
                <p className="text-xs text-gray-400 mt-0.5">Thiên đường thú cưng</p>
              </div>
            </Link>
            ...
            <div className="space-y-3">
              {[
                ...
                <a
                  key={i}
                  href={item.href}
                  className="flex items-center gap-2.5 text-xs text-gray-500 hover:text-[#1a3c5e] transition-colors group"
                >
                  <div
                    className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                    style={{ background: 'rgba(26,60,94,0.04)', border: '1px solid rgba(26,60,94,0.08)' }}
                  >
          ...
          <div className="lg:col-span-2">
            <h4 className="text-sm font-bold text-[#1a3c5e] mb-5 flex items-center gap-2">
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={item.label}>
                  <Link
                    to={item.path}
                    className="text-sm text-gray-500 hover:text-[#1a3c5e] transition-colors hover:translate-x-1 inline-block transform duration-200"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div className="lg:col-span-4">
            ...
            {subscribed ? (
              ...
              >
                <p className="text-emerald-600 font-semibold text-sm">🎉 Cảm ơn bạn đã đăng ký!</p>
                <p className="text-gray-400 text-xs mt-1">Kiểm tra email để nhận ưu đãi.</p>
              </div>
            ) : (
              <form onSubmit={handleSubscribe}>
                ...
                >
                  <input
                    ...
                    required
                    className="flex-1 bg-transparent text-sm text-gray-800 placeholder-gray-400 outline-none px-3 py-2"
                  />
                  <button
                    type="submit"
            ...
            <div className="flex items-center gap-3 mt-6">
              <p className="text-xs text-gray-400 font-medium">Theo dõi:</p>
              {[
        ...
        {/* Divider */}
        <div
          className="w-full h-px mb-6 bg-gray-100"
        />
