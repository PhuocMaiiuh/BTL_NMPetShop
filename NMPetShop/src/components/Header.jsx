...
import { FiUser, FiShoppingCart, FiSearch, FiLogOut, FiSettings, FiChevronDown, FiX } from 'react-icons/fi';
        ...
        <div className="flex items-center justify-between h-16">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <div key={link.label} className={`relative group py-5 -my-5 ${isDropdown ? 'min-w-[90px] flex justify-center' : ''}`}>
                  {link.submenu ? (
                    <span className={`relative cursor-pointer text-sm font-medium transition-colors duration-200 py-1 flex items-center gap-1 ${isDropdownActive ? 'text-primary' : 'text-text-gray hover:text-primary'}`}>
                      {currentLabel}
                      <FiChevronDown size={14} className="transition-transform group-hover:rotate-180 flex-shrink-0"/>
                      <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-primary transform origin-left transition-transform duration-300 ${isDropdownActive ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`}></span>
                    </span>
                  ) : (
                    <Link to={link.path} className={`relative text-sm font-medium transition-colors duration-200 py-1 flex items-center gap-1 ${isActive ? 'text-primary' : 'text-text-gray hover:text-primary'}`}>
                      {link.label}
                    ...
                    <div className="absolute top-full left-0 mt-0 min-w-[120px] bg-white rounded-xl shadow-lg border border-border py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 translate-y-2 group-hover:translate-y-0">
                      {link.submenu.map((subItem) => (
                        <Link
                          key={subItem.label}
                          to={subItem.path}
                          className={`block px-4 py-2 text-sm transition-colors ${location.search === '?' + subItem.path.split('?')[1] ? 'text-primary bg-bg-gray font-medium' : 'text-text-gray hover:bg-bg-gray hover:text-primary'}`}
                        >
                          {subItem.label}
                        </Link>
                      ))}
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden lg:block" ref={searchRef}>
              <div className="flex items-center bg-bg-gray rounded-full px-4 py-2 gap-2">
                <FiSearch className="text-text-gray shrink-0" size={16}/>
                <input
                  type="text"
                ...
                {searchQuery && (
                  <button
                    onClick={clearSearch}
                    className="text-text-gray hover:text-text-dark transition-colors shrink-0"
                    title="Xóa tìm kiếm"
                  >
                    <FiX size={14}/>
                  </button>
                  ...
                  {filteredProducts.length > 0 ? (
                    <>
                      ...
                      <button
                        ...
                        className="w-full text-center text-xs text-primary hover:bg-bg-gray py-2 font-medium transition-colors"
                      >
            ...
            <Link to="/gio-hang" className="p-2 text-text-gray hover:text-primary transition-colors duration-200 relative hover:bg-bg-gray rounded-full">
            ...
            {isAuthenticated ? (
              <div className="relative" ref={dropdownRef}>
                ...
                {showDropdown && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-border py-2 z-50">
                    ...
                    <Link to="/ho-so" onClick={() => setShowDropdown(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm text-text-gray hover:bg-bg-gray hover:text-text-dark transition-colors">
                      <FiUser size={16}/> Hồ sơ của tôi
                    </Link>
                    {isAdmin && (
                      <Link to="/admin" onClick={() => setShowDropdown(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm text-text-gray hover:bg-bg-gray hover:text-text-dark transition-colors">
                        <FiSettings size={16}/> Quản trị Admin
                      </Link>
                    )}
                    <div className="border-t border-border mt-1 pt-1">
                      <button onClick={handleLogout} className="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-danger hover:bg-danger/5 transition-colors">
              ...
              <Link to="/dang-nhap" className="p-2 text-text-gray hover:text-primary transition-colors duration-200 hover:bg-bg-gray rounded-full ml-2">
                <FiUser size={20}/>
              </Link>
            )}
          </div>
